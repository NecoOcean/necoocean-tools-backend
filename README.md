...
《系统设计文档》开发顺序已在本地完成前六步：日志骨架、统一契约、数据层（Flyway + JPA）、认证会话、公开读、留言提交（C-07、C-04b）。公网地址对同一工具 60 秒最多 3 条，同一地址 60 秒最多 5 条；回环和内网地址不计入。关键词只标记，不拦截。`GET /download/{file_id}` 在对象存储接入前对已上架文件返回 `40405`。下一步是后台管理。
